...
const EMPTY_FORM = { name: "", price: "", description: "", category: "Mains", imageUrl: "", available: true };
...
const MenuTab = () => {
  ...
  const toggleAvailable = async (dish) => {
    await updateDoc(doc(db, "menu", dish.id), { available: !dish.available });
  };

  const handleDelete = async (id) => {
    const dish = dishes.find(d => d.id === id);
    // Try to delete image from Storage too if it's a Firebase Storage URL
    if (dish?.imageUrl?.includes("firebasestorage")) {
      try {
        const imgRef = ref(storage, dish.imageUrl);
  ...
  const inputCls = "w-full bg-[#1a1a1a] border border-white/10 text-white placeholder-white/20 text-sm px-3 py-2.5 focus:outline-none focus:border-[#fa5631]/60 transition-colors";
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          { label: "Available",    val: dishes.filter(d => d.available !== false).length },
          { label: "Hidden",       val: dishes.filter(d => d.available === false).length },
        ].map(({ label, val }) => (
          <div key={label} className="bg-[#111111] border border-white/5 px-4 py-3">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-white/20 text-sm">
          {dishes.length === 0 ? "No dishes yet — add your first dish!" : "No dishes match your search."}
        </div>
      ) : (
          ...
          {filtered.map(dish => (
            <div key={dish.id}
              className={`bg-[#111111] border transition-all duration-200 ${
                dish.available === false ? "border-white/5 opacity-60" : "border-white/5 hover:border-white/10"
              }`}
            >
              <div className="grid grid-cols-2 md:grid-cols-12 gap-3 items-center px-4 py-3">
                <div className="md:col-span-1">
                  {dish.imageUrl ? (
                    <img src={dish.imageUrl} alt={dish.name} loading="lazy" className="w-10 h-10 object-cover" />
                  ) : (
                ...
                <div className="md:col-span-3">
                  ...
                  <p className="text-white/30 text-xs truncate max-w-[180px]">{dish.description}</p>
                </div>
                <div className="md:col-span-2">
                  <span className="text-xs text-white/40 bg-white/5 px-2 py-0.5">{dish.category}</span>
                </div>
                <div className="md:col-span-2">
                  <span className="text-[#fa5631] font-bold text-sm">₦{Number(dish.price || 0).toLocaleString()}</span>
                </div>
                <div className="md:col-span-2">
                  <button onClick={() => toggleAvailable(dish)}
                  ...
                  >{dish.available === false ? "Hidden" : "Available"}</button>
                </div>
                <div className="md:col-span-2 flex items-center justify-end gap-2">
                  ...
                  <button onClick={() => setDeleteConfirm(dish.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative z-10 bg-[#111111] border border-white/10 w-full max-w-lg p-7 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-white font-bold text-base">{editingId ? "Edit Dish" : "Add New Dish"}</h3>
              <button onClick={closeForm} className="text-white/30 hover:text-white bg-transparent border-none cursor-pointer">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <input type="text" required value={form.name} placeholder="e.g. Jollof Rice"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" required min="0" value={form.price} placeholder="5000"
              ...
              <div>
                ...
                <textarea rows={3} value={form.description} placeholder="Brief description of the dish..."
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  {["upload", "url"].map(mode => (
                    <button key={mode} type="button"
                      onClick={() => { setImageMode(mode); setImageFile(null); setImagePreview(mode === "url" ? form.imageUrl : ""); }}
                ...
                {imageMode === "upload" ? (
                  <div>
                    ...
                    >
                      {imagePreview ? (
                        <img src={imagePreview} alt="preview" className="mx-auto max-h-32 object-contain mb-2" />
                      ) : (
                        <div className="text-white/20">
                          ...
                          <p className="text-[10px] mt-1 text-white/10">JPG, PNG, WEBP — max 5MB</p>
                        </div>
                      )}
                      {imageFile && (
                        <p className="text-[#fa5631] text-xs mt-2 truncate">{imageFile.name}</p>
                      )}
                    </div>
                    ...
                    {uploading && (
                      <div className="mt-2">
                        ...
                        <div className="h-1 bg-white/10">
                          <div className="h-1 bg-[#fa5631] transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <input type="url" value={form.imageUrl} placeholder="https://example.com/image.jpg"
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-white/50 text-sm">{form.available ? "Available on menu" : "Hidden from menu"}</span>
              </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative z-10 bg-[#111111] border border-white/10 w-full max-w-sm p-7">
            <h3 className="text-white font-bold text-base mb-2">Delete dish?</h3>
            <p className="text-white/40 text-sm mb-6">This cannot be undone. The dish and its image will be permanently removed.</p>
